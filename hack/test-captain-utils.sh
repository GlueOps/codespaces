...
for c in git yq jq helm; do command -v "$c" >/dev/null || { echo "hack/test-captain-utils.sh: $c is required" >&2; exit 1; }; done
echo "== bash -n"; bash -n .devcontainer/tools/captain_utils.sh .devcontainer/libexec/captain_utils/crds .devcontainer/libexec/captain_utils/custom.sh "$TESTS"/*.sh hack/test-captain-utils.sh
...
# finishes in seconds. CI runs it as its own job; locally, CRDS_TEST_KIND=1 hack/test-captain-utils.sh.
