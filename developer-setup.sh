...
dev() {
    ...
    if sudo docker ps -a --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
        ...
        if [ "${ENVIRONMENT:-prod}" = "nonprod" ]; then
            ...
            # --- Check Cached Images ---
            local cached_images
            cached_images=$(sudo docker images --format "{{.Repository}}:{{.Tag}}" | grep "public-mirror.gpkg.io/glueops/mirror/glueops/codespaces")
        ...
        if ! gum spin --spinner dot --title "Creating container '$CONTAINER_NAME'..." --show-output -- \
            sudo docker run -itd --name "$CONTAINER_NAME" \
                ...
                "public-mirror.gpkg.io/glueops/mirror/glueops/codespaces:${CONTAINER_TAG_TO_USE}" bash; then
             gum style --padding "0 1" --foreground=196 --bold \
...
if [ -z "$GLUEOPS_CODESPACES_CONTAINER_TAG" ]; then
  ...
  # The glueops/mirror replication has a delay of 5-20minutes. This re-tagging is to leverage the public-mirror endpoint and still be able to use an existing cache.
  sudo docker tag ghcr.io/glueops/codespaces:$GLUEOPS_CODESPACES_CONTAINER_TAG public-mirror.gpkg.io/glueops/mirror/glueops/codespaces:$GLUEOPS_CODESPACES_CONTAINER_TAG
fi
