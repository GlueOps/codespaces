...
# Trap for better error debugging
...
if [[ -z "$BASTION_HOST" || -z "$TARGET_IP" || -z "$LOCAL_PEM" ]]; then
    echo "Usage: $0 <BASTION_IP> <TARGET_IP> <LOCAL_PEM> [SSH_FLAGS]"
    echo "Example: $0 157.180.22.11 157.180.11.11 bastion.pem -L 6443:127.0.0.1:6443"
    exit 1
fi

# Validate PEM file exists
if [[ ! -f "$LOCAL_PEM" ]]; then
    echo "❌ Error: PEM file not found: $LOCAL_PEM"
    exit 1
fi
